...
import xyz.taskov1ch.auction.database.AuctionRepository;
...
public class AuctionService {
	...
	public boolean buyNow(Player buyer, long id) {
		try {
			...
			int updated = repository.tryBuyNow(id, price, buyer.getUniqueId().toString(), price);
			if (updated == 0) {
	...
	public boolean forceExpire(long id) {
		try {
			...
			int updated = repository.updateStatusIfActive(auction.getId(), AuctionStatus.SOLD);
			if (updated > 0) {
				addClaim(auction.getBidderUuid(), auction.getItemNbt(), 0, "item");
				double taxPercent = configManager.getAuctionTaxPercent();
				double taxAmount = auction.getCurrentPrice() * (taxPercent / 100.0);
				double payout = Math.max(0, auction.getCurrentPrice() - taxAmount);
	...
	public boolean placeBid(Player bidder, long id, double amount) {
		try {
			AuctionItem auction = repository.getAuction(id);
			if (auction == null || !AuctionStatus.ACTIVE.name().equals(auction.getStatus())) {
				return false;
			...
			double minStep = configManager.getMinBidStep();
			double minPrice = auction.getCurrentPrice() + minStep;
			if (amount < minPrice) {
				return false;
			}
			if (!economyHook.isAvailable() || !economyHook.has(bidder, amount)) {
				return false;
			}

			String prevBidder = auction.getBidderUuid();
			double prevPrice = auction.getCurrentPrice();
			if (!economyHook.withdraw(bidder, amount)) {
				return false;
			}

			int updated = repository.updateBidIfActive(id, amount, bidder.getUniqueId().toString(), prevPrice);
			if (updated == 0) {
			...
			if (prevBidder != null) {
				Player prev = getOnlinePlayer(prevBidder);
				if (prev != null) {
					economyHook.deposit(prev, prevPrice);
				} else {
					addClaim(prevBidder, null, prevPrice, "refund");
				}
			}

			if (auction.getBuyoutPrice() != null && amount >= auction.getBuyoutPrice()) {
				finishAuction(auction, bidder, amount, AuctionStatus.SOLD, true);
			}
			return true;
		} catch (Exception e) {
			plugin.getLogger().warning("Place bid failed: " + e.getMessage());
			return false;
		}
	}
