...
public class AuctionRepository {
	...
	public long createAuction(AuctionItem item) {
		String sql = "INSERT INTO auction_items (seller_uuid, seller_name, item_nbt, item_name, start_price, current_price, buyout_price, bidder_uuid, status, created_at, end_at) "
				+
				"VALUES (:seller_uuid, :seller_name, :item_nbt, :item_name, :start_price, :current_price, :buyout_price, :bidder_uuid, :status, :created_at, :end_at)";
		try (Connection con = databaseProvider.getDatabase().openConnection()) {
			return con.createQuery(sql, true)
					...
					.addParameter("bidder_uuid", item.getBidderUuid())
	...
	public int updateBidIfActive(long id, double currentPrice, String bidderUuid, double expectedPrice) {
		String sql = "UPDATE auction_items SET current_price = :current_price, bidder_uuid = :bidder_uuid "
				+ "WHERE id = :id AND status = :status AND current_price = :expected_price";
		try (Connection con = databaseProvider.getDatabase().openConnection()) {
			return con.createQuery(sql)
					.addParameter("current_price", currentPrice)
					.addParameter("bidder_uuid", bidderUuid)
					.addParameter("id", id)
					.addParameter("status", AuctionStatus.ACTIVE.name())
	...
	public int tryBuyNow(long id, double currentPrice, String bidderUuid, double expectedPrice) {
		String sql = "UPDATE auction_items SET current_price = :current_price, bidder_uuid = :bidder_uuid, "
	...
	private String auctionItemColumns() {
		return "id, seller_uuid AS sellerUuid, seller_name AS sellerName, item_nbt AS itemNbt, item_name AS itemName, "
				...
				+ "bidder_uuid AS bidderUuid, status, created_at AS createdAt, end_at AS endAt";
	}
