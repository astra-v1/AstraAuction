...
один основной (со стандартными репозиториями Lumi) и второй с суффиксом `.old` (без репозиториев Lumi, всё через `jitpack.io`).

Если репозитории Lumi временно недоступны (например, ошибка 500) и сборка падает, используйте `build.gradle.old`.
Для этого достаточно временно удалить (или переместить) текущий `build.gradle`, а у `build.gradle.old` удалить суффикс `.old`.
